...
export const usePatientStore = create(
  persist(
    (set, get) => ({
      ...
      // Sync reports count with server
      syncReportsWithServer: async (serverTotalReports, eventType) => {
        const { currentPatient, reports } = get();
        if (!currentPatient) return;
        if (reports.length === serverTotalReports) return;
        // If event is delete and server count < local, fetch reports
        if (eventType === 'delete' && serverTotalReports < reports.length) {
          set({ reportsLoading: true });
          await get().fetchReports(currentPatient.id);
          set({ reportsLoading: false });
        }
        // If event is add and server count === local + 1, fetch reports
        if (eventType === 'add' && serverTotalReports === reports.length + 1) {
          set({ reportsLoading: true });
          await get().fetchReports(currentPatient.id);
          set({ reportsLoading: false });
        }
