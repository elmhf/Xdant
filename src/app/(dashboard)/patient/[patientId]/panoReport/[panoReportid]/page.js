...
export default function PanoReportPage() {
  ...
  const panoReportId = params.panoReportid;
  ...
  useEffect(() => {
    ...
    if (!panoReportId || panoReportId === lastProcessedId.current) {
      return;
    }
    ...
    console.log('🚀 Fetching new report:', panoReportId);
    lastProcessedId.current = panoReportId;

    // Simple fetch without complex state management
    fetchData(panoReportId).catch(error => {
      console.error('❌ Fetch failed:', error);
    });

  }, [panoReportId, imageCard, fetchData]);

  // Loading state
  if (loading) {
    return (
      <ReportLoading message="Loading Panoramic Report..." reportId={panoReportId} imageCard={imageCard} icon="🦷" />
    );
  }
    ...
    return (
      <div className="flex w-full flex-col items-center justify-center min-h-screen gap-4 p-6 max-w-md mx-auto">
        ...
        <h2 className="text-xl font-semibold text-red-600">
          Failed to Load Panoramic Report
        </h2>
  ...
  if (hasData && data) {
    ...
    return (
      <div className="w-full max-h-full mx-auto max-w-[90%] sm:w-full">
        <Dashboard reportType={reportType || "pano"} reportData={data} />
      </div>
    );
  }
  ...
  const handleLoadReport = () => {
    ...
    lastProcessedId.current = null;
    if (panoReportId && imageCard) {
      fetchData(panoReportId);
    }
  };
    ...
    <div className="flex flex-col w-screen items-center justify-center min-h-screen gap-6 p-6">
      ...
      <div className="text-center">
        <h2 className="text-lg font-medium text-gray-600 mb-2">
          No Panoramic Report Data Available
        </h2>
        <p className="text-sm text-gray-500 max-w-md">
          The panoramic report could not be loaded. This might be due to network issues or the report may not exist.
        </p>
      </div>
