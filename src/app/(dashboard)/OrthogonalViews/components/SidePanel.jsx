    ...
    <div className="relative flex flex-row rounded-[0.5vw] overflow-hidden items-start h-full">
      ...
      <div className="max-h-[100%] h-full bg-white px-2 py-4 flex flex-col gap-2 items-center relative  z-10">
        <motion.div
          ...
          className="absolute left-0 w-1 bg-[#0d0c22] rounded-r-md"
        ...
        {icons.map((item) => (
          <button
            ...
            className={`relative flex items-center justify-center w-10 h-10 rounded-md transition-colors duration-300 ${active === item.id ? "bg-[#0d0c22] text-white" : "bg-gray-100 text-[#0d0c22]"
              }`}
            title={item.label}
          >
            {item.icon}
            {active === item.id && (
              <motion.span
                ...
                className="absolute inset-0 rounded-md bg-[#0d0c22] z-[-1]"
                transition={{ type: "spring", stiffness: 500, damping: 30 }}
              />
            )}
          </button>
