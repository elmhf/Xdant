...
// مكون فرعي لرسم الفك
const JawRenderer = React.memo(({ jawData, showSettings, scale }) => {
  ...
  return (
    <>
      {/* الفك العلوي */}
      {showSettings?.Jaw?.showUpperJaw && jawData.upperJaw?.mask?.length > 0 && (
        <Line
          points={adjustCoordinates(jawData.upperJaw.mask)}
      ...
      {/* الفك السفلي */}
      {showSettings?.Jaw?.showLowerJaw && jawData.lowerJaw?.mask?.length > 0 && (
        <Line
          points={adjustCoordinates(jawData.lowerJaw.mask)}
          {...createLineProps(COLOR_PALETTE.jaw, scale)}
          closed
        />
      )}
