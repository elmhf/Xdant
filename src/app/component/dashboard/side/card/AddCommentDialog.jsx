      ...
      <DialogContent forceMount className="w-full max-w-xl bg-white rounded-2xl border border-gray-100 p-0 shadow-2xl">
        ...
        <div className="flex items-center justify-between px-8 pt-7 pb-3 border-b border-gray-100">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <span className="font-bold text-xl text-black select-none ml-2">Tooth {toothInfo.toothNumber}</span>
          </div>
        </div>
              ...
              {notes.map((n, idx) => (
                <li key={idx} className="bg-gray-50 rounded-xl border border-gray-100 shadow-sm p-4 flex flex-col relative">
                  ...
                  {editIdx === idx ? (
                    <>
                      <textarea
                        className="w-full border border-gray-200 rounded-lg p-2 text-sm mb-2 bg-white focus:border-black focus:ring-2 focus:ring-black/10"
                        value={editText}
                      ...
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <label className="text-gray-400 hover:text-black cursor-pointer text-xs">
                          <Paperclip size={16} />
                    ...
                    <>
                      ...
                      <div className="flex flex-row gap-2 justify-end mt-2 w-full">
                        <button onClick={() => handleEdit(idx)} className="p-1 text-gray-400 hover:text-black transition">
                          <Pencil size={18} />
                        </button>
                        <button onClick={() => handleDelete(idx)} className="p-1 text-gray-400 hover:text-black transition">
                          <Trash size={18} />
                        </button>
                      </div>
                    </>
 ...
 */}
        <div className="px-8 pt-2 pb-2">
          <div className="relative">
            <textarea
              ...
              className="w-full min-h-[120px] max-h-60 border-none rounded-2xl p-5 text-base font-medium text-black bg-white focus:outline-none focus:ring-2 focus:ring-black/10 resize-y transition-all duration-200 placeholder:text-gray-400 pr-16 shadow-sm"
        ...
        <div className="flex items-center justify-end gap-3 px-8 pb-7 pt-4 mt-6">
          {/* Attachment Button */}
          <label className="p-2 rounded-lg bg-gray-50 border border-gray-100 text-gray-500 hover:bg-gray-100 hover:text-black transition cursor-pointer" tabIndex={-1}>
            <Paperclip size={22} />
            <input
              type="file"
              className="hidden"
          ...
          <button
            className="rounded-xl border border-gray-200 bg-white text-black font-semibold text-base px-7 py-2 transition-all hover:bg-gray-100 shadow-sm"
          ...
          <button
            className="rounded-xl bg-black text-white font-semibold text-base px-7 py-2 transition-all hover:bg-gray-900 hover:text-white border border-black flex items-center justify-center min-w-[90px] shadow-sm"
            onClick={handleSend}
